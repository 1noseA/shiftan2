...
| ロール | 担当画面 |
|---|---|
| office | スタッフ管理（登録・編集・無効化）のみ |
| manager | 設定（基本設定・勤務パターン・必要人数・自動生成条件・人間関係制約）、希望休一覧、シフト（生成・編集・公開） |
| staff | 希望休入力、シフト閲覧 |

機能ごとの画面を最小化し、staff と manager で共有する画面（希望休・シフト）は **同一URLでロール別に挙動を切り替える** 方針とする。  
合計10画面：
...
| C-05 | ダッシュボード | `/dashboard` | 全ロール | ロール別メニュー表示 |
| F-01 | 設定 | `/settings` | manager | タブで全設定を1画面に統合 |
| F-02 | 希望休 | `/day-off` | manager / staff | staff:入力 / manager:一覧 |
...
タブで全設定を1画面に統合。manager のみアクセス可能。

| タブ | 内容 | 操作 |
|---|---|---|
| 基本設定 | 希望休締切日、希望休上限日数 | 編集（全店共通） |
| 勤務パターン | 勤務パターンCRUD（全店共通） | 新規・編集・有効/無効切替 |
| 必要人数 | 平日・休日 × 勤務パターン × 自部門 | 自店舗自部門 編集 |
| 自動生成条件 | 制約フラグ × 自部門 | 自店舗自部門 編集 |
| 人間関係制約 | スタッフペア × 自部門 | 自店舗自部門 編集・有効/無効切替 |

##### 共通動作

- 各タブ内で保存ボタンを押すと該当データのみ更新（タブ切替で未保存変更があれば確認）
- 必要人数・自動生成条件・人間関係制約は manager の自店舗自部門で固定表示
...
- アクション：保存
- 補足：全店共通シングルトン。複数のmanagerが編集する可能性があるため、最終更新者と更新日時を表示
...
- 表示・入力：平日・休日 × 勤務パターンのマトリクス（自部門固定）
- アクション：保存
...
ダッシュボード（C-05）のロール別メニューと同内容。常時表示。
