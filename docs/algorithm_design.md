...
| H2 1日1シフト | `enable_one_shift_per_day` | 制約を加えない（同日複数割当を許可） |
...
```python
if settings.enable_one_shift_per_day:
...
| `UNKNOWN` | なし | `200 infeasible` | × | タイムアウトかつ解未発見 |
| `INFEASIBLE` | なし | `200 infeasible` | × | 制約矛盾。原因分析を返す（5.3 / 5.4） |
...
| `INFEASIBLE` | INFEASIBLE / UNKNOWN（解なし） |
...
CP-SAT が `INFEASIBLE` を返した場合、ハード制約を順に緩和して再求解し、ボトルネックを特定する：
...
タイムアウト時の挙動は [5.1 ソルバーステータス別の判定](#51-ソルバーステータス別の判定) に従う：UNKNOWN かつ実行可能な中間解が見つかっていれば `success`（警告付き）として保存、見つかっていなければ `infeasible` として失敗扱い。
...
-- 1. 既存 shifts を行ロック付きで取得（並行生成リクエストの競合回避）
...
- `FOR UPDATE` で同一 (target_year_month, store_id, department_id) への並行生成リクエストの競合を排除
...
| ソルバータイムアウトかつ実行可能解なし | `200 infeasible`（reasons は段階的緩和診断の結果） |
