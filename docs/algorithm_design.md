...
CP-SAT の `solver.Solve()` の戻り値が `INFEASIBLE` の場合、または探索タイムアウトで `UNKNOWN` の場合は失敗とする。
...
タイムアウト時は `UNKNOWN` を返すため、最良解（`solver.BestObjectiveBound()` 等）が見つかっていればそれを利用する。
...
1. `BEGIN`
2. 既存 `shift_assignments` を削除（同一 shift_id 配下、`overwrite_existing=true` 時のみ）
3. `shifts` を UPSERT（`updated_at = now()`）
4. `shift_assignments` を bulk INSERT（`assignment_type='auto'`）
5. `COMMIT`

失敗時は `ROLLBACK`。`shift_id` と `assignment_count` をレスポンスに返す。
