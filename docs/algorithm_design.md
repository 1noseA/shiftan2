...
CP-SAT の前段で、簡易な候補数チェックを行う。候補集合の絞り込み条件はモデルと同じフラグを参照し、OFF の制約は除外しない：

```
∀(d, p):
    candidates[d, p] = {s ∈ S
        | (enable_workable_pattern が OFF、または wp[s] = p)
        | (enable_day_off_hard が OFF、または (d, s) ∉ DOff)
    }
...
`/shifts/generate` レスポンスの `reasons[]` に以下を含める（[api_design.md 4.2](./api_design.md#42-post-shiftsgenerate) と整合）。各 reason は対応する制約フラグが ON のときのみ報告する：

| type | 報告条件 | 集計内容 |
|---|---|---|
| `staff_shortage` | 常時 | `|candidates[d, p]| < req[d, p]` の (d, p) を抽出 |
| `day_off_excess` | `enable_day_off_hard` が ON のときのみ | 候補から希望休でブロックされたスタッフ数を `shortage_breakdown.day_off_blocked` に集計 |
| `pattern_mismatch` | `enable_workable_pattern` が ON のときのみ | 候補から勤務パターン不一致でブロックされたスタッフ数を `pattern_mismatch_blocked` に集計 |
| `consecutive_limit` | `enable_max_consecutive` が ON のときのみ | CP-SAT 失敗時、制約緩和診断で連勤制限が原因の (d, p) を集計 |
